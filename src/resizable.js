...
export default Extension.create({
    ...
    onCreate({ editor }) {
            ...
            if (/bottom/.test(e.target.className)) {
                ...
                document.addEventListener("mouseup", () => {
                    ...
                    editor.commands.updateAttributes('image', {
                        width: resizeLayer.style.width,
                        height: resizeLayer.style.height,
                    })
        ...
        for (let name of handlers) {
            ...
            item.style.zIndex = "1000000000";
            Object.entries(this.options.handlerStyle).forEach(([key, value]) => {
                item.style[key] = value;
            });
            const dir = name.split("-");
            item.style[dir[0]] = parseInt(item.style.width) - 20 + "px";
            item.style[dir[1]] = parseInt(item.style.height) - 20 + "px";
